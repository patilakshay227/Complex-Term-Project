...
c.execute("select C.userID, count(distinct C.assetURL) as artCount from comments C, commenterGender CG, articles A where C.assetURL = A.webURL and C.userID = CG.userID and C.username = CG.username and CG.gender = 'female' group by C.userID order by artCount desc;")
