...
c.execute("SELECT C.userID, count(distinct section) as secCount from comments C, articles A where C.assetURL = A.webURL and section is not null group by C.userID order by secCount desc")
...
plt.title('Sections Distribution over Commenters')
